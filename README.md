...
- **Юридические документы** (условия использования, политика конфиденциальности,
  ...
  Ссылки задаются в админке, раздел «Тексты и настройки»: `terms_url`, `privacy_url`,
  `pd_consent_url`. Они подставляются в текст под приветствием и в ответы команд
  `/terms`, `/privacy`, `/pd_consent`. Пока ссылка пустая, показывается текст-заглушка.
